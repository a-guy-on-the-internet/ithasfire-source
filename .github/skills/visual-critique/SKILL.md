...
- Model: `agy`'s default (a Gemini Flash tier) sees images fine; pass `--model`
  with an exact string from `agy models` (has spaces/parens — quote it) to use
  Gemini Pro for a deeper pass.
